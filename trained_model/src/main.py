...
class FilePaths:
    ...
    fn_char_list = 'trained_model/model/charList.txt'
    fn_summary = 'trained_model/model/summary.json'
    fn_corpus = 'trained_model/data/corpus.txt'
...
def infer(model: Model, img) -> None:
...
def init_model():
    ...
    action='store_true'
    model = Model(list(open(FilePaths.fn_char_list).read()), DecoderType.WordBeamSearch, must_restore=True, dump='store_true')
    return model
def get_predict_from_roi(roi,model):
    return infer(model, roi)
def main():
    """Main function."""
    ...
    # infer text on test image
        ...
        return infer(model, img_predict)
